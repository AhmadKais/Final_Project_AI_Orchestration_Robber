...
- [x] **Closed the gap flagged above**: `SeriesRunner` now writes all four Sec. 9.3 artifacts, not just log+results. `_write_declaration` (once, from sub-game 1's real Step-0 exchange) and `_write_config_snapshot` (once per sub-game, byte-identical here but separately named as the spec requires) added directly to the production code, not a one-off script. Team-member IDs and repo URL come from `config/<role>/game.toml`'s **already-scaffolded** `[game]` `members`/`repos` fields (found and fixed a real key-name mismatch here: the new code first read a nonexistent `repo_url` key instead of the existing `repos = {cop, thief}` structure) -- left at their shipped `TODO-*` placeholder values, the declaration's identity fields are just blank rather than fabricated; fill in real data there before an actual submission.
- [x] `scripts/generate_sample_series_report.py` + `docs/sample_reports/series/`: a full real 6-game series played through the actual `SeriesRunner` (`MinimaxBrain` both sides, role alternation), producing all four artifact types for every sub-game -- complements the existing single-game sample in `docs/sample_reports/`.
- [x] 215 tests, 214 passing, 1 skipped.

## Real 2-process network test found and fixed a genuine connection-race bug ✅
- [x] Ran two fully separate `police_thief peer` processes against each other over real localhost sockets (not the in-process test transport `test_orchestrator_integration.py`/`test_series_runner.py` use) -- the one dimension 215 in-process tests structurally cannot exercise, since none of them go through a real ASGI server startup sequence. Found: the first connection attempt reliably failed with `RuntimeError: FastMCP's StreamableHTTPSessionManager task group was not initialized` -- FastMCP's own "Uvicorn running" log line prints before its session manager finishes initializing, so an opponent starting at the same moment (the normal way a real match begins) loses that race almost every time, and `OpponentClient` had zero retry logic, so this crashed the whole peer outright before the first move.
- [x] Fixed in `infra/mcp_client.py`: `OpponentClient._call` retries a connection-level failure until `response_timeout_sec` elapses instead of raising immediately; a genuine tool-level timeout (McpError code 408) still fails fast, unchanged. Added `tests/test_mcp_infra.py::test_connection_refused_retries_until_the_opponent_starts_listening` and `::test_nothing_ever_listening_times_out_bounded_not_a_hang` (real TCP sockets, not mocks) proving both the retry-then-succeed and retry-then-bounded-timeout paths.
- [x] Re-ran the exact two-process scenario that found the bug: now completes a full real game end to end (`Game over: capture` on both sides, 300+ real HTTP requests exchanged) instead of crashing before Step-0 even finished.
- [x] Known cosmetic-only remainder: a harmless `RuntimeError` can print in a peer's log during its own shutdown if the opponent's final session-termination request arrives mid-teardown -- confirmed this only ever happens strictly after `Game over: ...` already printed, so it never affects the outcome, score, or any written artifact.
- [x] 217 tests, 216 passing, 1 skipped.

## Wired up the send-report step ✅
- [x] `send_report()`/`Gatekeeper` existed since Stage 7 but neither was ever actually called from a real workflow -- no CLI path, nothing invoking `Gatekeeper.try_send`. Found while documenting "how do we run it with another opponent" and closed it.
- [x] Real correctness catch before writing any code: the shipped `SCOPES` is `gmail.send`-only (least privilege, Appendix A Sec. 1C). Gmail's own `drafts.create` endpoint needs the broader `gmail.compose`/`gmail.modify` scope, so a same-scope "Gmail draft" mode would 403 the moment it actually ran. Redesigned "draft" to mean a **local `.eml` file** instead -- `email_sender.save_local_draft` writes the exact MIME message next to the report and never calls the Gmail API at all, so it needs neither a broader scope nor `credentials.json`/`token.json`.
- [x] `email_sender.send_or_draft_report(service, gatekeeper, mode, ...)`: `mode = "send"` routes the real Gmail call through `gatekeeper.try_send` (raises if any gate blocks it -- a block means something's actually wrong, not a silent no-op); any other mode, including the default `"draft"`, writes the local `.eml` and skips the gate entirely (nothing external to rate-limit).
- [x] `infra/gatekeeper.build_gatekeeper(rate_limiter_config)`: builds a real `Gatekeeper` from `config/game.json`'s `rate_limiter_gatekeeper` section (`requests_per_minute` -> token-bucket capacity/refill, `queue_depth` doubles as the daily cap since the section has no explicit one).
- [x] `simulation_sdk.find_report_artifacts`/`run_report`: locates every Sec. 9.3 artifact `SeriesRunner` already wrote for a series (declaration, per-sub-game configs, per-sub-game logs, results) and reports each one, reading `[email] recipient`/`mode` from `config/<role>/game.toml`. New CLI: `police_thief report --role <police|thief>`.
- [x] New tests: `tests/test_email_sender.py` (`save_local_draft`, `send_or_draft_report` dispatch and gate-block behavior), `tests/test_gatekeeper.py::test_build_gatekeeper_*`, `tests/test_run_report.py` (artifact discovery + ordering, draft mode never touches `get_service`, send mode does, missing-recipient/no-artifacts error paths) -- all mocked, zero real Gmail credentials or network, same philosophy as the existing suite.
- [x] Left untouched, on purpose: no real send or Gmail draft was fired against the user's actual account as part of this work -- `[email] mode` ships as `"draft"` in both `config/<role>/game.toml`, so the very first real invocation of `police_thief report` writes local `.eml` previews, not a live email.
- [x] 230 tests, 229 passing, 1 skipped.
...
- [ ] End-of-match email sent by both sides, separately -- the mechanism is now wired and tested (`police_thief report --role <role>`, see "Wired up the send-report step" above); actually firing it for real still depends on those real matches happening first, plus flipping `[email] mode` from the shipped `"draft"` default to `"send"`
